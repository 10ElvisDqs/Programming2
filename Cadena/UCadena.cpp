...
void CCadena::Elim_Palabras_No_Comiencen_SubCadena(UnicodeString subcadena){
	...
	// Eliminar el último espacio en blanco de la cadena resultante (si lo hay)
...
// Convertir letra a minúscula si es mayúscula
	...
	for (int i = 0; i < n; i++) {
		char c = Cadena[i];
		// Convertir carácter a minúscula si es mayúscula
		if (c >= 'A' && c <= 'Z') {
			c += ('a' - 'A');
		}
		// Si el carácter es diferente a la letra, copiarlo en la posición j
		if (c != letra) {
			Cadena[j] = Cadena[i];
			j++;
		}
